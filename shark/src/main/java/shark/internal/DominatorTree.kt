...
@Doc("如果我们把Java对象之间的引用关系看做一张有向图（可以存在环）的话，对象的支配树体现了对象之间的支配关系。如果所有指向对象B的路径都要经过对象A，则认为对象A支配对象B。如果对象A是离对象B最近的支配对象，则认为对象A是对象B的直接支配者")
...
//                            println("dominator leaf: ${if (dominator == ValueHolder.NULL_REFERENCE) "null" else nameOfHeapObject(graph.findObjectById(dominator))}")
     ...
     * first search（广度优先搜索）, so when objectId already has a known dominator then its dominator path is
