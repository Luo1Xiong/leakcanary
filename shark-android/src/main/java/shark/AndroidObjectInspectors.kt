...
enum class AndroidObjectInspectors : ObjectInspector {
    ...
    ANDROIDX_FRAGMENT {
        override val leakingObjectFilter = { heapObject: HeapObject ->
            val result = heapObject is HeapInstance &&
                    heapObject instanceOf "androidx.fragment.app.Fragment" &&
                    heapObject.getOrThrow(
                            "androidx.fragment.app.Fragment", "mFragmentManager"
                    ).value.isNullReference

//            if (heapObject is HeapInstance && heapObject instanceOf "androidx.fragment.app.Fragment")
//                println("check if fragment is leaking >> ${heapObject.instanceClassName} ${heapObject.objectId} $result")
            result
        }
    ...
    companion object {
         ...
         */
        @KApi("" +
                "https://www.kotlincn.net/docs/reference/collection-transformations.html" +
        ...
        fun createLeakingObjectFilters(inspectors: Set<AndroidObjectInspectors>): List<LeakingObjectFilter> =
                inspectors.mapNotNull {
                    it.leakingObjectFilter
                }.map { filter ->
...
/**
 * Same as [HeapInstance.readField] but throws if the field does'nt exist
 */
