...
class HprofRecordReader internal constructor(
    ...
    @Doc("6.一个ID占用一个identifierByteSize，一个u4占用一个INT_SIZE(4字节)，一个u2占用一个SHORT_SIZE(2字节)，一个u1占用一个字节")
    ...
    fun skipClassDumpConstantPool() {
        ...
        for (i in 0 until constantPoolCount) {
            // constant pool index
            skip(SHORT.byteSize)                  // 下标
            val typeOfEntry = readUnsignedByte()  // 基础类型
            skip(sizeOf(typeOfEntry))             // 实体
        }
    }
    ...
    fun readUnsignedByte(): Int {
        val iii = readByte().toInt()
//        println(">>> $iii ${Integer.toBinaryString(iii)} ${Integer.toBinaryString(BYTE_MASK)} ${Integer.toBinaryString(iii and BYTE_MASK)} <<< ")
        return iii and BYTE_MASK
    }
