...
internal class UnsortedByteEntries(
    ...
    @Doc("多个subArray组成一个entries，只能通过append方法来新增entry，" +
            "subArray填充完毕后的总长度大多数都小于bytesPerEntry，因为bytesPerEntry指定的是所有entry中最大的一个entry所占据的byteCount，因此这必定会造成内存占用的提高，但却最大化了访问速度")
    private var entries: ByteArray? = null
    private val subArray = MutableByteSubArray()

    @Doc("9.**下次**将要写入的subArray的index")
        ...
        ByteArrayTimSort.sort(entries, 0, assigned, bytesPerEntry, object : ByteArrayComparator {
            override fun compare(
                ...
                return if (longIdentifiers) {
                    readLong(o1Array, o1Index * entrySize).compareTo(readLong(o2Array, o2Index * entrySize))
                } else {
