...
sealed class HeapObject {
    ...
    @Important("1.从hprof文件中读取所有对象到内存中，每个record都有编号，总共有四中类型的record，" +
            "他们按照次序存储在heapGraph中，分别是classes、instances、object arrays、primitive arrays")
    /**
     ...
     */
    val asClass: HeapClass? get() = if (this is HeapClass) this else null

    /**
     * This [HeapObject] as a [HeapInstance] if it is one, or null otherwise
     */
    val asInstance: HeapInstance? get() = if (this is HeapInstance) this else null

    /**
     * This [HeapObject] as a [HeapObjectArray] if it is one, or null otherwise
     */
    val asObjectArray: HeapObjectArray? get() = if (this is HeapObjectArray) this else null

    /**
     * This [HeapObject] as a [HeapPrimitiveArray] if it is one, or null otherwise
     */
    val asPrimitiveArray: HeapPrimitiveArray? get() = if (this is HeapPrimitiveArray) this else null
    ...
    class HeapInstance internal constructor(
            ...
            override val objectId: Long,  // INSTANCE ID
            override val objectIndex: Int
    ) : HeapObject() {
         ...
         */
        val isPrimitiveWrapper: Boolean get() = instanceClassName in primitiveWrapperClassNames

        override val graph: HeapGraph get() = hprofGraph

        /**
         * @see HeapClass.instanceByteSize
         */
        val byteSize get() = instanceClass.instanceByteSize

        /**
         * The name of the class of this instance, identical to [Class.getName].
         */
        val instanceClassName: String get() = hprofGraph.className(indexedObject.classId)

        /**
         * Returns [instanceClassName] stripped of any string content before the last period (included).
         */
        val instanceClassSimpleName: String get() = classSimpleName(instanceClassName)

        /**
         * The class of this instance.
         */
        val instanceClass: HeapClass get() = hprofGraph.findObjectById(indexedObject.classId) as HeapClass

        /**
         * The heap identifier of the class of this instance.
         */
        val instanceClassId: Long get() = indexedObject.classId
         ...
         * This may trigger IO reads.
         */
        @Important("读取变量，包括父类的所有变量,flatten()用于将所有读取到的变量组成一个序列" +
                "sequences在操作大量数据时比直接操作集合本身有更好的性能")
        fun readFields(): Sequence<HeapField> {
            val fieldReader by lazy {
                hprofGraph.createFieldValuesReader(readRecord())
            }
            val allHeapFieldOfThisClass = instanceClass.classHierarchy
                    .map { heapClass ->
...
//                                    if (heapClass.simpleName.contains("bitmap", true)) {
//                                        println("readFields > className:${heapClass.simpleName}")
//                                    }
//                                    if (fieldName.contains("bitmap", true)) {
//                                        allBitmapTypes.add(fieldName)
//                                        if (fieldValue is ReferenceHolder) {
//                                            if (fieldValue.value != 0L) {
//                                                println("readFields > fieldValue is ${fieldValue.value}")
//                                                a++
//                                                if (321300720L == fieldValue.value)
//                                                    println("readFields > find $fieldName 321300720L")
//                                                else if (333785440L == fieldValue.value)
//                                                    println("readFields > find $fieldName 333785440L")
//                                                else if (333784680L == fieldValue.value)
//                                                    println("readFields > find $fieldName 333784680L")
//                                                else if (331444768L == fieldValue.value)
//                                                    println("readFields > find $fieldName 331444768L")
//                                                else if (330093984L == fieldValue.value)
//                                                    println("readFields > find $fieldName 330093984L")
//                                            }
//                                            b++
//                                        }
//                                    }
                    ...
                    .flatten()
//            println("a:$a | b:$b | allBitmapTypes:${Arrays.toString(allBitmapTypes.toArray())}")
            return allHeapFieldOfThisClass
        }
        ...
        override fun toString(): String {
            return "$instanceClassName@$objectId"
        }
    }
    ...
    class HeapObjectArray internal constructor(
         ...
         */
        val arrayClassName: String get() = hprofGraph.className(indexedObject.arrayClassId)
        ...
        override fun toString(): String {
            return "$arrayClassName@$objectId"
        }
    }
    ...
    companion object {
        ...
        var a = 0
        var b = 0
        val allBitmapTypes = HashSet<String>()
