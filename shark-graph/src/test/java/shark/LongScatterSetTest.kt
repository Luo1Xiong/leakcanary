...
class LongScatterSetTest {
    ...
    @Test
    fun `new set is empty`() {
        assertThat(LongScatterSet())
                .isEmpty()
    }

    @Test
    fun `LongScatterSet#add() adds elements`() {
    ...
    @Test
    fun `+= operator works as addition`() {
        val set = LongScatterSet()
    ...
    @Test
    fun `when adding element twice, it is only added once`() {
        val set = LongScatterSet()
     ...
     */
    @Test
    fun `11 and 14_723_950_898 have same hash`() {
        Assertions.assertThat(mixPhi(11))
                .isEqualTo(mixPhi(14_723_950_898))
    }

    @Test
    fun `elements with equal hash can be added`() {
        val set = LongScatterSet()
    ...
    @Test
    fun `LongScatterSet#remove() removes elements`() {
        val set = LongScatterSet()
        TEST_VALUES_LIST.forEach { set.add(it) }
    ...
    @Test
    fun `elements with equal hash can be removed`() {
        val set = LongScatterSet()
        set.add(SAME_MIX_PHI_1)
        set.add(SAME_MIX_PHI_2)
    ...
    @Test
    fun `LongScatterSet#release() empties set`() {
        val set = LongScatterSet()
        set.add(TEST_VALUE)
     ...
     */
    @Test
    fun `setting initial capacity after operations`() {
        val set = LongScatterSet()
        set.add(TEST_VALUE)
    ...
    @Test
    fun `adding a lot of elements causes resizing`() {
        val set = LongScatterSet()
